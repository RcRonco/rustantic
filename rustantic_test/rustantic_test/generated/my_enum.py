...
class MyEnumD(BaseModel):
    ...
    value: NoneType = Field(default=None) # NoneType
...
class MyEnum(RootModel[MyEnumType]):
    ...
    def to_rs(self):
        inner_to_rs = getattr(self.root.value, "to_rs", None)
        if inner_to_rs:
           # If `to_rs` exists, run it to get pyo3 object
           val: Any = inner_to_rs()
        else:
           # Otherwise, assume the value is native python and should work when passed to pyo3
           val = self.root.value
